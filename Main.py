...
def flightSimulation():
    global simulation_window, ani
    simulation_window = Toplevel(main)
    simulation_window.title("Flight Simulation Dashboard")
...
def explainModel():
    ...
    background = all_X_train[np.random.choice(all_X_train.shape[0], 100, replace=False)]
    explainer = shap.KernelExplainer(svm_cls.predict, background)
    shap_values = explainer.shap_values(all_X_test[:10])
    
    # Create SHAP summary plot
    plt.figure()
    shap.summary_plot(shap_values, all_X_test[:10], feature_names=all_data.columns)
    plt.title("Feature Impact on Predictions (SHAP)")
    plt.tight_layout()
    plt.show()
    
    text.insert(END,"SHAP analysis completed. Check the generated plot!\n")


def interactiveConfusionMatrix():
    ...
    ConfusionMatrixDisplay.from_predictions(all_y_test, predict, ax=ax)
    ax.set_title("Interactive Confusion Matrix")
    
    canvas = FigureCanvasTkAgg(fig, master=main)
    canvas.get_tk_widget().place(x=900, y=300)
    canvas.draw()
...
simButton = Button(main,text="Flight Simulation", command=flightSimulation, bg='#ffb3fe')
simButton.place(x=900,y=100)
simButton.config(font=font1)

shapButton = Button(main,text="Explain Model (SHAP)", command=explainModel, bg='#ffb3fe')
shapButton.place(x=900,y=150)
shapButton.config(font=font1)

matrixButton = Button(main,text="Confusion Matrix", command=interactiveConfusionMatrix, bg='#ffb3fe')
matrixButton.place(x=900,y=200)
